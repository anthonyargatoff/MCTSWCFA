...
public class PlayerSpriteController: SpriteControllerMonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (framesSinceLastWalkUpdate > GameManager.GetScaledFrameCount(FramesBetweenWalkUpdate))
        {
            ...
            if (controller.IsWalking)
            {
                AudioManager.PlaySound(Audios.Move, controller.UsingHammer ? 0f : 0.5f, pitch: pitches[walkPitchIndex]);
                walkPitchIndex++;
            ...
            {
                AudioManager.PlaySound(Audios.Ladder, controller.UsingHammer ? 0f: 0.5f, pitch: pitches[climbPitchIndex]);
                climbPitchIndex++;
